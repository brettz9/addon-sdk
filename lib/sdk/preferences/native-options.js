...
function injectOptions({ preferences, preferencesBranch, document, parent, id }) {
  ...
  function saveHotkey (name, textbox) {
    var hk = textbox.value;
    let str = Cc["@mozilla.org/supports-string;1"].
              createInstance(Ci.nsISupportsString);
    str.data = hk;
    branch.setComplexValue(name, Ci.nsISupportsString, str);
  ...
  for (let { name, type, hidden, title, description, label, options, on, off } of preferences) {
    ...
    if (type === 'file' || type === 'directory') {
      ...
      lbl.setAttribute('control', id);
      
      let textbox = document.createElement('textbox');
      textbox.setAttribute('id', id);
      textbox.setAttribute('preference', name);
      textbox.setAttribute('value', branch.getComplexValue(name, Ci.nsISupportsString).data);

      var KeySelector = require('sdk/keyboard/KeySelector');
      KeySelector.addListeners(textbox);
      
      saveHotkey(name, textbox);
      textbox.addEventListener('blur', function (e) { // Change event may fire to frequently given the added key commands
        saveHotkey(name, textbox);
      });

      setting.appendChild(lbl);
      setting.appendChild(textbox);
    }
    else if (type === 'radio') {
